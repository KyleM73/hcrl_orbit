...
class PathIntegralController:
    ...
    def __call__(self, obs: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        for i in range(self.step, self.num_steps):
            ...
            noisy_control = torch.einsum("ij,ajk->aik", self.G_u, self.s * self.eps_sampled[i-self.num_steps] * self.dt**0.5)
    ...
    def check_collision(self, obs: torch.Tensor, x: torch.Tensor, y: torch.Tensor, collision_flag: torch.Tensor):
        # check in bounds
        inside_border = (
