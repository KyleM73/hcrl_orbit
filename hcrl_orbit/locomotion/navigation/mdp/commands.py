...
from omni.isaac.orbit.utils.math import quat_rotate_inverse, wrap_to_pi, yaw_quat, quat_from_euler_xyz, quat_mul
...
class BoxPoseCommand(CommandTerm):
    ...
    def __init__(self, cfg: BoxPoseCommandCfg, env: BaseEnv):
        ...
        if self.cfg.normalized: 
            self.norm = torch.tensor([self.cfg.ranges.pos_x[1], self.cfg.ranges.pos_y[1], 3.14], device=self.device)
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if self.cfg.simple_heading:
            ...
            target_vec = self.pos_command_w[env_ids] - self.robot.data.body_pos_w[env_ids, self.body_id, :3]
    ...
    def _update_command(self):
        ...
        target_vec = self.pos_command_w - self.robot.data.body_pos_w[:, 0, :3]
        self.pos_command_b[:] = quat_rotate_inverse(yaw_quat(self.robot.data.body_quat_w[:, self.body_id, :]), target_vec)
        self.heading_command_b[:] = wrap_to_pi(self.heading_command_w - self.robot.data.heading_w)

    def _update_metrics(self):
        # logs data
        self.metrics["error_pos"] = torch.norm(self.pos_command_w[:, :2] - self.robot.data.body_pos_w[:, 0, :2], dim=1)
